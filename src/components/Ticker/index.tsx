...
import { Animated, Easing, StyleSheet, Text, View, TextStyle, TextProps, I18nManager } from 'react-native';

// Original module: https://github.com/browniefed/react-native-ticker
...
const TickItem: React.FC<TickProps> = ({ children, duration, textStyle, textProps, measureMap, rotateItems }) => {
...
const Ticker: React.FC<Props> = ({ duration = 250, textStyle, textProps, children }) => {
  ...
  return (
    <View style={styles.row}>
      ...
      {rotateItems.map((v) => {
        return (
          <Text key={v} {...textProps} style={[textStyle, styles.hide]} onLayout={(e) => handleMeasure(e, v)}>
            {v}
          </Text>
        );
      })}
